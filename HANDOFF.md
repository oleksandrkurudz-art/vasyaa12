...
- Згенерований клієнт — у `.gitignore` (не в git). Регенерується автоматично на `postinstall`,
  `build` і **`npm run dev`** (скрипт `predev` — додано після того, як стейл-клієнт мовчки давав 404
  на сторінках розділів/статей: `findUnique`/`findFirst` по `slug` повертали null). Якщо запускаєш
  dev НЕ через `npm run dev` — зроби `npx prisma generate` вручну після зміни схеми.
...
Спеціалізовані розділи (Афіша з датами, Вакансії…),
...
| `Sidebar.tsx` | Популярне 🔥 / Погода / Курси (**живі**: НБУ + open-meteo, кеш ~1год + фолбек) |
...
- Погода і курси валют у шапці/сайдбарі — **живі дані**: курси з API НБУ (`src/lib/rates.ts`,
  bank.gov.ua, без ключа), погода з open-meteo (`src/lib/weather.ts`, координати громади, без ключа).
  Обидва — власний TTL-кеш ~1год на рівні модуля (бо force-dynamic сторінки вимикають fetch-кеш Next)
  + фолбек на статичні значення, якщо API недоступний. Шапка й сайдбар беруть курс з одного джерела.
